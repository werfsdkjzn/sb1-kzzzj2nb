...
import { networkingTasks3 } from '../data/networkingCourse3';
import TaskContent from '../components/TaskContent';
import TaskList from '../components/TaskList';
import ProgressHeader from '../components/ProgressHeader';
import CongratsScreen from '../components/CongratsScreen';

export default function NetworkingCourse3() {
  ...
  const calculateProgress = () => {
    ...
    return (completedCount / networkingTasks3.length) * 100;
  };

  const handleNextTask = () => {
    if (currentTask < networkingTasks3.length - 1) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      <ProgressHeader
        ...
        subtitle="Aprenda os principais protocolos e suas aplicações em redes"
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {!showCongrats ? (
          <div className="grid md:grid-cols-[300px,1fr] gap-6">
            ...
            <TaskContent
              task={networkingTasks3[currentTask]}
              ...
              onAnswerSelect={(questionIndex: number, choiceId: string) => {
                ...
                const allQuestionsCorrect = networkingTasks3[currentTask].questions.every(question => {
              ...
              isLast={currentTask === networkingTasks3.length - 1}
            />
          </div>
        ) : (
          <CongratsScreen onReset={resetCourse} nextCourse="/networking4" />
        )}
      </main>
    </div>
  );
